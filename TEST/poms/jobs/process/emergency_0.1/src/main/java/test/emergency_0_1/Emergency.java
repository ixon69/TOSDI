...
public void tDBConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:Btjsr9q47tZCTjAWL6290EO+L3jEyBuangbU0C+CEuwwVQ==");
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_TEST_Emergency = new byte[0];
    static byte[] commonByteArray_TEST_Emergency = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_TEST_Emergency.length) {
				if(length < 1024 && commonByteArray_TEST_Emergency.length == 0) {
   					commonByteArray_TEST_Emergency = new byte[1024];
				} else {
   					commonByteArray_TEST_Emergency = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_TEST_Emergency, 0, length);
			strReturn = new String(commonByteArray_TEST_Emergency, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_TEST_Emergency) {
...
+"E\n  FROM DUAL\nUNION ALL\nSELECT OUTRMDD DD\n       ,TO_CHAR(COUNT(*)) CNT\n       ,TO_CHAR(SUM(CASE WHEN OUTRMCD = '31"
+"' THEN 1 ELSE 0 END))\n        ||'('||\n        TO_CHAR(SUM(CASE WHEN OUTRMCD = '31' AND SDDIAGYN = 'Y' THEN 1 ELSE 0 EN"
...
+"  ,TO_CHAR(ROUND(SUM(CASE WHEN BEDPOSCNTS NOT LIKE '%♣%' AND SDDIAGYN = 'Y' AND OUTRMCDNM IN ('수술', '입원') THEN OUTMODIN "
+"ELSE 0 END)\n                / SUM(CASE WHEN BEDPOSCNTS NOT LIKE '%♣%' AND SDDIAGYN = 'Y' AND OUTRMCDNM IN ('수술', '입원') "
+"THEN 1 ELSE 0 END),0)) CRINAVGTIME\n       ,'ICU중증 '||TO_CHAR(ROUND(SUM(CASE WHEN BEDPOSCNTS NOT LIKE '%♣%' AND SDDIAGYN"
+" = 'Y' AND OUTRMCD IN ('32', '34') THEN OUTMODIN ELSE 0 END)\n                / SUM(CASE WHEN BEDPOSCNTS NOT LIKE '%♣%' "
+"AND SDDIAGYN = 'Y' AND OUTRMCD IN ('32', '34') THEN 1 ELSE 0 END),0)) || '분' CRRMAVGTIME\n       ,TO_CHAR(SUM(CASE WHEN "
+"OUTROOMCD IN ('432','736','532','533','534','535','543','546','3074') THEN 1 ELSE 0 END)) BUFBED\n       ,TO_CHAR(ROUND("
+"SUM(CASE WHEN OUTROOMCD IN ('432','736','532','533','534','535','543','546','3074') THEN OUTMODIN ELSE 0 END)\n         "
+"             / SUM(CASE WHEN OUTROOMCD IN ('432','736','532','533','534','535','543','546','3074') THEN 1 ELSE 0 END),0)"
+") BUFTIME\nFROM EMER\nGROUP BY OUTRMDD";
...
+"YY-MM-DD') dd \n  FROM DUAL\nUNION ALL select '무증상버퍼' rmgbn, '432(4bed)' room, to_char(sum(case when OUTROOMCD = '432' t"
+"hen 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '736(4bed)' room, to_char(sum(case when OUTROOMCD = '736' th"
+"en 1 else 0 end)) dd from EMER\nUNION ALL select '무증상버퍼 sub ICU' rmgbn, '543(2bed)' room, to_char(sum(case when OUTROOMC"
+"D = '543' then 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '546(2bed)' room, to_char(sum(case when OUTROOMCD"
+" = '546' then 1 else 0 end)) dd from EMER\nUNION ALL select '무증상버퍼 ICU' rmgbn, '3074(3bed)' room, to_char(sum(case when "
+"OUTROOMCD = '3074' then 1 else 0 end)) dd from EMER\nUNION ALL select '유증상버퍼' rmgbn, '532(1bed)' room, to_char(sum(case "
+"when OUTROOMCD = '532' then 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '533(1bed)' room, to_char(sum(case w"
+"hen OUTROOMCD = '533' then 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '534(2bed)' room, to_char(sum(case wh"
+"en OUTROOMCD = '534' then 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '535(2bed)' room, to_char(sum(case whe"
+"n OUTROOMCD = '535' then 1 else 0 end)) dd from EMER\nUNION ALL select '' rmgbn, '535(2bed)' room, to_char(sum(case when"
+" OUTROOMCD = '535' then 1 else 0 end)) dd from EMER\nUNION ALL select '입원환자 통계' rmgbn, '입원환자수' room, to_char(sum(case wh"
+"en OUTROOMCD is not null then 1 else 0 end)) dd from EMER\nUNION ALL select '입원환자 통계' rmgbn, '중증상병환자' room, to_char(sum("
+"case when OUTROOMCD is not null and SDDIAGYN = 'Y' then 1 else 0 end)) dd from EMER\nUNION ALL select '입원환자 통계' rmgbn, '"
+"경증환자' room, to_char(sum(case when OUTROOMCD is not null and (SDDIAGYN != 'Y' OR SDDIAGYN is null) then 1 else 0 end)) dd"
+" from EMER\nUNION ALL select '입원환자 통계' rmgbn, '버퍼룸 입원 합계' room, to_char(sum(case when OUTROOMCD IN ('432','736','532','5"
+"33','534','535','543','546','3074') then 1 else 0 end)) dd from EMER\nUNION ALL select '입원환자 통계' rmgbn, '무증상버퍼' room, to"
+"_char(sum(case when OUTROOMCD IN ('432','736','543','546','3074') then 1 else 0 end)) dd from EMER\nUNION ALL select '입원"
+"환자 통계' rmgbn, '유증상버퍼' room, to_char(sum(case when OUTROOMCD IN ('532','533','534','535') then 1 else 0 end)) dd from EME"
+"R";
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Emergency.class.getClassLoader().getResourceAsStream("test/emergency_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
...
/************************************************************************************************
 *     130233 characters generated by Talend Open Studio for Data Integration 
 *     on the 2020년 11월 6일 (금) 오전 9시 18분 40초
 ************************************************************************************************/