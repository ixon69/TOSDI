...
package test.durtoexcelandctalk_0_1;
...
/**
 * Job: DurToExcelAndCtalk Purpose: export dur suspect<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class DurToExcelAndCtalk implements TalendJob {
	...
	private final String jobName = "DurToExcelAndCtalk";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					DurToExcelAndCtalk.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(DurToExcelAndCtalk.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String url_tDBConnection_1 = "jdbc:oracle:thin:@(description=(address=(protocol=tcp)(host="
						+ "172.17.71.152" + ")(port=" + "1521" + "))(connect_data=(service_name=" + "HIS014" + ")))";
				...
				final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:nl3a5xNHe6o2O+ov81yFG3EGeaSFvqy27k1/4ghZdUeeug==");
				...
				if ("noDatetimeStringSync=true" != null && !"\"\"".equals("noDatetimeStringSync=true")
						&& !"".equals("noDatetimeStringSync=true")) {
					atnParamsPrope_tDBConnection_1.load(new java.io.ByteArrayInputStream(
							"noDatetimeStringSync=true".replace("&", "\n").getBytes()));
				}
	...
	public static class userStruct implements routines.system.IPersistableRow<userStruct> {
		final static byte[] commonByteArrayLock_TEST_DurToExcelAndCtalk = new byte[0];
		static byte[] commonByteArray_TEST_DurToExcelAndCtalk = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_DurToExcelAndCtalk.length) {
					if (length < 1024 && commonByteArray_TEST_DurToExcelAndCtalk.length == 0) {
						commonByteArray_TEST_DurToExcelAndCtalk = new byte[1024];
					} else {
						commonByteArray_TEST_DurToExcelAndCtalk = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_DurToExcelAndCtalk, 0, length);
				strReturn = new String(commonByteArray_TEST_DurToExcelAndCtalk, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_DurToExcelAndCtalk) {
	...
	public void tDBInput_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String fileName_tFileOutputExcel_2 = "D:/data/DurBatch/해외감염병정보_"
						+ TalendDate.formatDate("yyyyMMdd", new Date()) + ".xls";
...
//create directory only if not exists		  
				...
				jxl.write.WritableFont wf_tFileOutputExcel_2 = new jxl.write.WritableFont(
						jxl.write.WritableFont.COURIER, 10, jxl.write.WritableFont.NO_BOLD, false);
...
//modif end
										user.GUBUN, format_tFileOutputExcel_2);
//modif start					
...
//modif ends							
								...
								int currentWith_0_tFileOutputExcel_2 = cell_0_tFileOutputExcel_2.getContents().trim()
										.length();
...
//modif end
										user.RSRVDD, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.ORDDEPTNM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.ORDDRNM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.PID, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.HNGNM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.RRGSTNO, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.HOMETEL, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.MPPHONTEL, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.ACPTDD, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet

//modif ends							
								writableSheet_tFileOutputExcel_2.addCell(cell_9_tFileOutputExcel_2);
								int currentWith_9_tFileOutputExcel_2 = cell_9_tFileOutputExcel_2.getContents().trim()
										.length();
...
//modif end
										user.ORDDD, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.ORDTM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.RSRVFLAG, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.FSEXAMFLAG, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.PRCPCD, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.PRCPNM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.EXECDEPTNM, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
//modif end
										user.REMFACT, format_tFileOutputExcel_2);
//modif start					
								// If we keep the cell format from the existing cell in sheet
...
// ###############################      
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final Process ps_tSystem_1 = runtime_tSystem_1.exec(
						"cmd /c java -jar CompactCTalkSender.jar 20800854 IXON@la5ohrs 5957083 D:/data/DurBatch/해외감염병정보_"
								+ TalendDate.formatDate("yyyyMMdd", new Date()) + ".xls",
						env_tSystem_1, new java.io.File(
								"D:/ProgramData/workspace-spring-tool-suite-4-4.6.0.RELEASE/CompactCTalkSender"));
	...
	public static void main(String[] args) {
		final DurToExcelAndCtalk DurToExcelAndCtalkClass = new DurToExcelAndCtalk();

		int exitCode = DurToExcelAndCtalkClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = DurToExcelAndCtalk.class.getClassLoader()
					.getResourceAsStream("test/durtoexcelandctalk_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = DurToExcelAndCtalk.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : DurToExcelAndCtalk");
		}
...
/************************************************************************************************
 * 105892 characters generated by Talend Open Studio for Data Integration on the
 * 2020년 6월 26일 (금) 오후 1시 13분 18초
 ************************************************************************************************/