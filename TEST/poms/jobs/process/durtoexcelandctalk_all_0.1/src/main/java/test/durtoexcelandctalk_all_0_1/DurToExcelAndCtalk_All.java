...
package test.durtoexcelandctalk_all_0_1;
...
/**
 * Job: DurToExcelAndCtalk_All Purpose: export dur suspect<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class DurToExcelAndCtalk_All implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "DurToExcelAndCtalk_All";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				DurToExcelAndCtalk_All.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(DurToExcelAndCtalk_All.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public void tDBConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:9MiVkwyi5ymR+ADr+N0GUWwEsnu9sTiOvUs6skS40j2tbg==");
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_TEST_DurToExcelAndCtalk_All = new byte[0];
    static byte[] commonByteArray_TEST_DurToExcelAndCtalk_All = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_TEST_DurToExcelAndCtalk_All.length) {
				if(length < 1024 && commonByteArray_TEST_DurToExcelAndCtalk_All.length == 0) {
   					commonByteArray_TEST_DurToExcelAndCtalk_All = new byte[1024];
				} else {
   					commonByteArray_TEST_DurToExcelAndCtalk_All = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_TEST_DurToExcelAndCtalk_All, 0, length);
			strReturn = new String(commonByteArray_TEST_DurToExcelAndCtalk_All, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_TEST_DurToExcelAndCtalk_All) {
...
+"CMPTBS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN("
+"BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HI"
+"STSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVF"
...
+"(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO"
+"1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL A"
...
+"'' PRCPCD,   --처방코드\n       '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICE"
+"COOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REM"
+"FACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHSCND B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD"
+" BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYY"
+"YMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSU"
+"KIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD"
+"\n   AND B.INSTCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FR"
+"OMDD AND B.TODD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIK"
+"A'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           A"
+"ND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAM"
+"E(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의"
+"사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' A"
+"S RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORD"
+"DD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM "
+"COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE"
+" WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드\n       '' PRCPNM, "
+"  --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP"
+"02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM"
+".PMCMPTBS E, PAM.PMCHRARE B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD"
+"') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG "
+"= '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKI"
+"ND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND B.INSTCD = E.INSTCD\n   AND B.R"
+"RGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FROMDD AND B.TODD\n   AND EXISTS \n      "
+" (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n     "
+"      AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT "
+"/*+ LEADING(EXOP OTPT OPRC) INDEX_SS(EXOP IX_MMODEXOP_09) */\n       DISTINCT '검사예약' AS GUBUN,\n       EXOP.RSRVDD,\n   "
+"    COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD, OTPT.ORDDEPTCD, OTPT.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(OTPT."
+"ORDDRID, OTPT.ORDDD) AS ORDDRNM,   --진료의사\n       OTPT.PID AS PID,    --등록번호\n       PTBS.HNGNM,  --성명\n       PTBS.RRGS"
+"TNO1 || SUBSTR(PTBS.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PTBS.HOMETEL AS HOMETEL,    -- 연락처\n       PTBS"
+".MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       OTPT.ORGORDDD AS ACPTDD,    --접수일자\n       OTPT.ORDDD AS ORDDD,    --진료일자\n  "
+"     OTPT.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = OTPT.RSRVFL"
+"AG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = OTPT.FSEXAMFLAG) AS "
+"FSEXAMFLAG,  --초재진\n       OPRC.PRCPCD,   --처방코드\n       OPRC.PRCPNM,   --처방명칭\n       COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD"
+", EXOP.EXECDEPTCD,OTPT.ORDDD) EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA'"
+" AND Y.TMP02 = PTBS.RRGSTNO1 AND Y.TMP03 = PTBS.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM EMR"
+".MMODEXOP EXOP, EMR.MMOHOPRC OPRC, PAM.PMOHOTPT OTPT, PAM.PMCMPTBS PTBS\n WHERE OTPT.INSTCD = NVL( '014', '014')\n   AND"
+" OTPT.HISTSTAT IN ('R','T')\n   AND OTPT.INSUKIND IN ('11','21','22')\n   AND OTPT.ORDDD BETWEEN TO_CHAR(ADD_MONTHS(TO_D"
+"ATE( '20200623','YYYYMMDD'), -36),'YYYYMMDD') AND '20200623' --3년데이터\n   AND OTPT.ORDTYPE != 'S'\n   AND OTPT.INSTCD = P"
+"TBS.INSTCD\n   AND OTPT.PID = PTBS.PID\n   AND OTPT.INSTCD = EXOP.INSTCD\n   AND OTPT.PID = EXOP.PID\n   AND OTPT.CRETNO"
+" = EXOP.ACTCRETNO\n   AND OTPT.ORDDD = EXOP.ACTORDDD\n   AND EXOP.INSTCD = OPRC.INSTCD\n   AND EXOP.PID = OPRC.PID\n   A"
+"ND EXOP.PRCPDD = OPRC.PRCPDD\n   AND EXOP.PRCPNO = OPRC.PRCPNO\n   AND EXOP.EXECPRCPHISTCD = OPRC.PRCPHISTCD\n   AND OPR"
+"C.PRCPHISTCD = 'O'\n   AND EXOP.EXECPRCPHISTCD = 'O'\n   AND EXOP.RSRVFLAG = 'Y' --예약검사여부\n   AND EXOP.EXECDD = '0000000"
+"0' --실시여부\n   AND EXOP.RSRVDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE B"
+"ASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND EXISTS\n       (SELECT Y.TMP04\n     "
+"     FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PTB"
+"S.RRGSTNO1\n           AND Y.TMP03 = PTBS.RRGSTNO2\n           AND Y.TMP10 = PTBS.PID)\nUNION ALL\nSELECT DISTINCT '검사예약"
+"' AS GUBUN,\n       A.EXECDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료"
+"과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID, A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       B.HNGNM, "
+" --성명\n       B.RRGSTNO1 || SUBSTR(B.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       B.HOMETEL AS HOMETEL,    -- 연락"
+"처\n       B.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n     "
+"  '' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n"
+"       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y."
+"TMP01 = 'ZIKA' AND Y.TMP02 = B.RRGSTNO1 AND Y.TMP03 = B.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1) AS REMFACT --전송내역\n  F"
+"ROM PAM.PAOHOSCL A, PAM.PMCMPTBS B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.EXECDD BETWEEN TO_CHAR(SYSDATE + 1, '"
+"YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND "
+"DUTFLAG = '0')\n   AND A.ORDTYPE != 'S'\n   AND A.CALCSTAT = 'Y'\n   AND A.INSUKIND = '11'\n   AND A.SUPPKIND != '01'\n "
+"  AND A.INSTCD = B.INSTCD\n   AND A.PID = B.PID\n   AND A.RCPTDD BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMM"
+"DD')-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND A.ORDDD BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1"
+", -12), 'YYYYMMDD') AND '20200623'-1\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         "
+"WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = B.RRGSTNO1\n           AND Y.TMP03 = B.RR"
+"GSTNO2\n           AND Y.TMP10 = B.PID)\n GROUP BY A.PID, A.ORDDD, A.CRETNO, A.INSTCD, A.ORDTYPE, EXECDD, RRGSTNO1||RRGS"
+"TNO2, HNGNM, A.ORDDEPTCD, A.SUPPKIND, A.INSUKIND, A.ORDDRID, B.RRGSTNO1, B.RRGSTNO2, B.HOMETEL, B.MPPHONTEL\nUNION ALL\n"
+"SELECT 'DSC예약' AS GUBUN,\n       A.INDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.INDD) AS ORDDE"
+"PTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.MEDISPCLID, A.INDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n    "
+"   E.HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMET"
+"EL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.INDD AS ACPTDD,    --접수일자\n       A.INDD AS ORDDD,  "
+"  --진료일자\n       A.INTM AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS"
+" PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SER"
+"VICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS"
+" REMFACT --전송내역\n   FROM PAM.PMIHINPT A, PAM.PMCMPTBS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.INDD BETWEEN TO_"
+"CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND "
+"HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT = 'Y'\n   AND A.ORDTYPE = 'D'\n   AND A.DSCHDD = '99991231'\n   AND"
+" A.CALCDD = '00000000' --입실체크 안된내역만.\n   AND A.CALCTM = '0000'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND "
+"= '51'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.S"
+"ERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n      "
+"     AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '입원예약' AS GUGUN,\n       A.PA"
+"TINHOPEDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.PATINHOPEDD) AS ORDDEPTNM, --진료과\n       COM"
+".FN_ZS_GETUSERNM(A.ORDDRID, A.INDIRECDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n  "
+"     E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n      "
+" E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,    --접수일자\n       A.PATINHOPEDD AS ORDDD,    --진료일자\n       '"
+"' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n   "
+"    '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP"
+"01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FRO"
+"M PAM.PMIHADRV A, PAM.PMCMPTBS B, PAM.PMIHINPT C, PAM.PMCMPTBS E\n WHERE A.PID = B.PID\n   AND A.INSTCD = B.INSTCD\n   A"
+"ND A.INSTCD = '014'\n   AND A.INSTCD = E.INSTCD\n   AND A.PID = E.PID\n   AND C.INSTCD(+) = A.INSTCD\n   AND C.PID(+) = "
+"A.PID\n   AND C.HISTSTAT(+) = 'Y'\n   AND C.INDSCHACPTSTAT(+) = 'A'\n   AND C.DSCHDD(+) = '99991231'\n   AND C.ORDTYPE(+"
+") = 'I'\n   AND A.INRSRVSTAT IN ('S','R')\n   AND A.INFLAG = 'G'\n   AND A.ERREGLYN IN ('I','F','R','G','D')\n   AND A.P"
+"ATINHOPEDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYS"
+"DATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVI"
+"CECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n          "
+" AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(ACPT.ACPT"
+"_DD, 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, '"
+"YYYYMMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRN"
+"M,   --진료의사\n       ACPT.PID AS PID,    --등록번호\n       PATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM."
+"RES_NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPP"
+"HONTEL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YY"
+"YYMMDD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT_TM AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHE"
+"RE GRUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n       (CASE WHEN ACPT.RMDEM_YN = 'Y' THEN '재"
+"진' ELSE '초진' END) AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXEC"
+"DEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y."
+"TMP03 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC."
+"HCADOTPT OTPT\n WHERE ACPT.INST_CD = NVL( '014', '014')\n   AND ACPT.ACPT_DD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AN"
+"D (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0'"
+")\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n   AND EXISTS \n       (SELECT Y.TMP04\n          FRO"
+"M PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO"
+"1\n           AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT IN ('C')--A 가예약, B 가예약"
+"취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   AND ACPT.HC_ACPT_SEQ_NO = O"
+"TPT.HC_ACPT_SEQ_NO\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(RCNT.RSLT_CNST_DD, 'YYYYMMDD') AS RSRVDD,\n      "
+" COM.FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n  "
+"     COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID"
+",    --등록번호\n       PATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO, "
+"  --주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(A"
+"CPT.ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n     "
+"  OTPT.OPD_MEDTRT_TM AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL "
+"= ACPT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n       '상담' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS P"
+"RCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA'"
+" AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HC"
+"AMRCNT RCNT, HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC.HCADOTPT OTPT\n WHERE RCNT.INST_CD = NVL( '014', '014')\n   AND RCNT"
+".RSLT_CNST_DD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR("
+"SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND RCNT.RSLT_CNST_VALI_CNST_CD = 'O'\n   AND EXISTS \n  "
+"     (SELECT 1\n          FROM HC.HCBMCODE SUB\n         WHERE SUB.INST_CD = RCNT.INST_CD\n           AND SUB.GRUP_CD_ID"
+" = 'A0003'\n           AND SUB.CD_VAL_NM LIKE '%내원%'\n           AND SUB.USE_YN = 'Y'\n           AND RCNT.RSLT_CNST_DD "
+"BETWEEN SUB.USE_FRDD AND SUB.USE_TODD\n           AND SUB.CD_VAL = RCNT.RSLT_CNST_FLAG)\n   AND RCNT.INST_CD = ACPT.INST"
+"_CD\n   AND RCNT.HC_ACPT_SEQ_NO = ACPT.HC_ACPT_SEQ_NO\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n "
+"  AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND "
+"Y.TMP11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PA"
+"TM.PID)\n   AND ACPT.HC_STAT NOT IN ('B','D','F')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   "
...
userStruct user = new userStruct();
	...
	currentComponent="tFileOutputExcel_2";
		...
		String fileName_tFileOutputExcel_2="D:/data/DurBatch/해외감염병정보_"+TalendDate.formatDate("yyyyMMdd", new Date()) + ".xls";
...
+"BS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASE"
+"DD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTST"
+"AT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG "
+"= '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND EXISTS \n      (SELECT Y.TMP04\n         FROM PAM.SERV"
+"ICECOOP Y\n        WHERE Y.TMP01 = 'ZIKA'\n          AND Y.TMP11 = 'Y'\n          AND Y.TMP02 = E.RRGSTNO1\n          AN"
+"D Y.TMP03 = E.RRGSTNO2\n          AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS R"
+"SRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.O"
+"RDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 ||"
+" SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MP"
+"PHONTEL,    -- 휴대폰\n       A.ORGORDDD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,"
+"    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n  "
+"     (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' P"
+"RCPCD,   --처방코드\n       '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP"
+" Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT"
+" --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHSCND B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BET"
+"WEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMD"
+"D') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSUKIND"
+" IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   "
+"AND B.INSTCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FROMDD"
+" AND B.TODD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n"
+"           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y"
+".TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A."
+"INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n "
+"      A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RR"
+"GSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORDDD A"
+"S ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM."
+"ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHE"
+"RE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드\n       '' PRCPNM,   --"
+"처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 ="
+" E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMC"
+"MPTBS E, PAM.PMCHRARE B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') A"
+"ND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0"
+"')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND ="
+" '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND B.INSTCD = E.INSTCD\n   AND B.RRGST"
+"NO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FROMDD AND B.TODD\n   AND EXISTS \n       (SE"
+"LECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n         "
+"  AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT /*+ "
+"LEADING(EXOP OTPT OPRC) INDEX_SS(EXOP IX_MMODEXOP_09) */\n       DISTINCT '검사예약' AS GUBUN,\n       EXOP.RSRVDD,\n       "
+"COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD, OTPT.ORDDEPTCD, OTPT.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(OTPT.ORDD"
+"RID, OTPT.ORDDD) AS ORDDRNM,   --진료의사\n       OTPT.PID AS PID,    --등록번호\n       PTBS.HNGNM,  --성명\n       PTBS.RRGSTNO1"
+" || SUBSTR(PTBS.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PTBS.HOMETEL AS HOMETEL,    -- 연락처\n       PTBS.MPP"
+"HONTEL AS MPPHONTEL,    -- 휴대폰\n       OTPT.ORGORDDD AS ACPTDD,    --접수일자\n       OTPT.ORDDD AS ORDDD,    --진료일자\n      "
+" OTPT.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = OTPT.RSRVFLAG) "
+"AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = OTPT.FSEXAMFLAG) AS FSEX"
+"AMFLAG,  --초재진\n       OPRC.PRCPCD,   --처방코드\n       OPRC.PRCPNM,   --처방명칭\n       COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD, EX"
+"OP.EXECDEPTCD,OTPT.ORDDD) EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND"
+" Y.TMP02 = PTBS.RRGSTNO1 AND Y.TMP03 = PTBS.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM EMR.MMO"
+"DEXOP EXOP, EMR.MMOHOPRC OPRC, PAM.PMOHOTPT OTPT, PAM.PMCMPTBS PTBS\n WHERE OTPT.INSTCD = NVL( '014', '014')\n   AND OTP"
+"T.HISTSTAT IN ('R','T')\n   AND OTPT.INSUKIND IN ('11','21','22')\n   AND OTPT.ORDDD BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE("
+" '20200623','YYYYMMDD'), -36),'YYYYMMDD') AND '20200623' --3년데이터\n   AND OTPT.ORDTYPE != 'S'\n   AND OTPT.INSTCD = PTBS."
+"INSTCD\n   AND OTPT.PID = PTBS.PID\n   AND OTPT.INSTCD = EXOP.INSTCD\n   AND OTPT.PID = EXOP.PID\n   AND OTPT.CRETNO = E"
+"XOP.ACTCRETNO\n   AND OTPT.ORDDD = EXOP.ACTORDDD\n   AND EXOP.INSTCD = OPRC.INSTCD\n   AND EXOP.PID = OPRC.PID\n   AND E"
+"XOP.PRCPDD = OPRC.PRCPDD\n   AND EXOP.PRCPNO = OPRC.PRCPNO\n   AND EXOP.EXECPRCPHISTCD = OPRC.PRCPHISTCD\n   AND OPRC.PR"
+"CPHISTCD = 'O'\n   AND EXOP.EXECPRCPHISTCD = 'O'\n   AND EXOP.RSRVFLAG = 'Y' --예약검사여부\n   AND EXOP.EXECDD = '00000000' -"
+"-실시여부\n   AND EXOP.RSRVDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASED"
+"D > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND EXISTS\n       (SELECT Y.TMP04\n         "
+" FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PTBS.RR"
+"GSTNO1\n           AND Y.TMP03 = PTBS.RRGSTNO2\n           AND Y.TMP10 = PTBS.PID)\nUNION ALL\nSELECT DISTINCT '검사예약' AS"
+" GUBUN,\n       A.EXECDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n "
+"      COM.FN_ZS_GETUSERNM(A.ORDDRID, A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       B.HNGNM,  --성"
...
+"1 = 'ZIKA' AND Y.TMP02 = B.RRGSTNO1 AND Y.TMP03 = B.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1) AS REMFACT --전송내역\n  FROM "
+"PAM.PAOHOSCL A, PAM.PMCMPTBS B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.EXECDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYY"
+"MMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTF"
+"LAG = '0')\n   AND A.ORDTYPE != 'S'\n   AND A.CALCSTAT = 'Y'\n   AND A.INSUKIND = '11'\n   AND A.SUPPKIND != '01'\n   AN"
+"D A.INSTCD = B.INSTCD\n   AND A.PID = B.PID\n   AND A.RCPTDD BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')"
+"-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND A.ORDDD BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1, -1"
+"2), 'YYYYMMDD') AND '20200623'-1\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHER"
+"E Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = B.RRGSTNO1\n           AND Y.TMP03 = B.RRGSTN"
+"O2\n           AND Y.TMP10 = B.PID)\n GROUP BY A.PID, A.ORDDD, A.CRETNO, A.INSTCD, A.ORDTYPE, EXECDD, RRGSTNO1||RRGSTNO2"
+", HNGNM, A.ORDDEPTCD, A.SUPPKIND, A.INSUKIND, A.ORDDRID, B.RRGSTNO1, B.RRGSTNO2, B.HOMETEL, B.MPPHONTEL\nUNION ALL\nSELE"
+"CT 'DSC예약' AS GUBUN,\n       A.INDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.INDD) AS ORDDEPTNM"
+", --진료과\n       COM.FN_ZS_GETUSERNM(A.MEDISPCLID, A.INDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       E"
+".HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL, "
+"   -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.INDD AS ACPTDD,    --접수일자\n       A.INDD AS ORDDD,    --"
+"진료일자\n       A.INTM AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS PRC"
+"PCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICE"
+"COOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REM"
+"FACT --전송내역\n   FROM PAM.PMIHINPT A, PAM.PMCMPTBS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.INDD BETWEEN TO_CHAR"
+"(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HIST"
+"STAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT = 'Y'\n   AND A.ORDTYPE = 'D'\n   AND A.DSCHDD = '99991231'\n   AND A.C"
+"ALCDD = '00000000' --입실체크 안된내역만.\n   AND A.CALCTM = '0000'\n   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '5"
+"1'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVI"
+"CECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n          "
+" AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '입원예약' AS GUGUN,\n       A.PATINH"
+"OPEDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.PATINHOPEDD) AS ORDDEPTNM, --진료과\n       COM.FN_"
+"ZS_GETUSERNM(A.ORDDRID, A.INDIRECDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n      "
+" E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.M"
...
+"M.PMIHADRV A, PAM.PMCMPTBS B, PAM.PMIHINPT C, PAM.PMCMPTBS E\n WHERE A.PID = B.PID\n   AND A.INSTCD = B.INSTCD\n   AND A"
+".INSTCD = '014'\n   AND A.INSTCD = E.INSTCD\n   AND A.PID = E.PID\n   AND C.INSTCD(+) = A.INSTCD\n   AND C.PID(+) = A.PI"
+"D\n   AND C.HISTSTAT(+) = 'Y'\n   AND C.INDSCHACPTSTAT(+) = 'A'\n   AND C.DSCHDD(+) = '99991231'\n   AND C.ORDTYPE(+) = "
+"'I'\n   AND A.INRSRVSTAT IN ('S','R')\n   AND A.INFLAG = 'G'\n   AND A.ERREGLYN IN ('I','F','R','G','D')\n   AND A.PATIN"
+"HOPEDD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE"
+", 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECO"
+"OP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND"
+" Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(ACPT.ACPT_DD,"
+" 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYY"
+"MMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,  "
+" --진료의사\n       ACPT.PID AS PID,    --등록번호\n       PATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_"
+"NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPPHONT"
+"EL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMM"
+"DD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT_TM AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE G"
+"RUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n       (CASE WHEN ACPT.RMDEM_YN = 'Y' THEN '재진' E"
+"LSE '초진' END) AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPT"
+"NM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP0"
+"3 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC.HCAD"
+"OTPT OTPT\n WHERE ACPT.INST_CD = NVL( '014', '014')\n   AND ACPT.ACPT_DD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (S"
+"ELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n "
+"  AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PA"
+"M.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO1\n "
+"          AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT IN ('C')--A 가예약, B 가예약취소, "
+"C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   AND ACPT.HC_ACPT_SEQ_NO = OTPT."
+"HC_ACPT_SEQ_NO\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(RCNT.RSLT_CNST_DD, 'YYYYMMDD') AS RSRVDD,\n       COM"
+".FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n      "
+" COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID,   "
+" --등록번호\n       PATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO,   --"
+"주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(ACPT."
+"ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n       OT"
+"PT.OPD_MEDTRT_TM AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL = AC"
+"PT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n       '상담' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPN"
+"M,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND"
+" Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HCAMRC"
+"NT RCNT, HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC.HCADOTPT OTPT\n WHERE RCNT.INST_CD = NVL( '014', '014')\n   AND RCNT.RSL"
+"T_CNST_DD BETWEEN TO_CHAR(SYSDATE + 1, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSD"
+"ATE, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND RCNT.RSLT_CNST_VALI_CNST_CD = 'O'\n   AND EXISTS \n      "
+" (SELECT 1\n          FROM HC.HCBMCODE SUB\n         WHERE SUB.INST_CD = RCNT.INST_CD\n           AND SUB.GRUP_CD_ID = '"
+"A0003'\n           AND SUB.CD_VAL_NM LIKE '%내원%'\n           AND SUB.USE_YN = 'Y'\n           AND RCNT.RSLT_CNST_DD BETW"
+"EEN SUB.USE_FRDD AND SUB.USE_TODD\n           AND SUB.CD_VAL = RCNT.RSLT_CNST_FLAG)\n   AND RCNT.INST_CD = ACPT.INST_CD"
+"\n   AND RCNT.HC_ACPT_SEQ_NO = ACPT.HC_ACPT_SEQ_NO\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n   AN"
+"D EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TM"
+"P11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PATM.P"
+"ID)\n   AND ACPT.HC_STAT NOT IN ('B','D','F')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   AND "
+"ACPT.INST_CD = OTPT.INST_CD\n   AND ACPT.HC_ACPT_SEQ_NO = OTPT.HC_ACPT_SEQ_NO";
...
final Process ps_tSystem_1 = runtime_tSystem_1.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar 20800854 IXON@la5ohrs 5728038 D:/data/DurBatch/해외감염병정보_"+TalendDate.formatDate("yyyyMMdd", new Date()) + ".xls" ,env_tSystem_1,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
...
final Process ps_tSystem_3 = runtime_tSystem_3.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar 20800854 IXON@la5ohrs 5728019 D:/data/DurBatch/해외감염병정보_"+TalendDate.formatDate("yyyyMMdd", new Date()) + ".xls" ,env_tSystem_3,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
...
final Process ps_tSystem_2 = runtime_tSystem_2.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar 20800854 IXON@la5ohrs 5728019 D:/data/DurBatch/CompactCTalkSender/none.txt" ,env_tSystem_2,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
...
final Process ps_tSystem_4 = runtime_tSystem_4.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar 20800854 IXON@la5ohrs 5728038 D:/data/DurBatch/CompactCTalkSender/none.txt" ,env_tSystem_4,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
 ...
 */
    ...
    public static void main(String[] args){
        final DurToExcelAndCtalk_All DurToExcelAndCtalk_AllClass = new DurToExcelAndCtalk_All();

        int exitCode = DurToExcelAndCtalk_AllClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = DurToExcelAndCtalk_All.class.getClassLoader().getResourceAsStream("test/durtoexcelandctalk_all_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = DurToExcelAndCtalk_All.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DurToExcelAndCtalk_All");
        }
...
/************************************************************************************************
 *     151967 characters generated by Talend Open Studio for Data Integration 
 *     on the 2020년 6월 29일 (월) 오전 8시 41분 19초
 ************************************************************************************************/