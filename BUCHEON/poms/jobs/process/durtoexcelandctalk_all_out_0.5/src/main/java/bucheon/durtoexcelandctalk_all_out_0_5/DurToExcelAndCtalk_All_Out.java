...
package bucheon.durtoexcelandctalk_all_out_0_5;
...
/**
 * Job: DurToExcelAndCtalk_All_Out Purpose: export dur suspect<br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class DurToExcelAndCtalk_All_Out implements TalendJob {
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext(){
			...
			if(out_user != null){
					...
					this.setProperty("out_user", out_user.toString());
				
			}
			
			if(out_password != null){
				
					this.setProperty("out_password", out_password.toString());
				
			}
			
			if(out_roomno != null){
				
					this.setProperty("out_roomno", out_roomno.toString());
...
public String getOut_password(){
	return this.out_password;
}
public String out_roomno;
public String getOut_roomno(){
	return this.out_roomno;
}
	}
	...
	private final String jobName = "DurToExcelAndCtalk_All_Out";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				DurToExcelAndCtalk_All_Out.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(DurToExcelAndCtalk_All_Out.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public void tDBConnection_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tDBConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:wTNpkdMs4KQtGMh98j2aCJUTPXAiM1Dnco73MxJI4CQrmg==");
...
public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
    final static byte[] commonByteArrayLock_BUCHEON_DurToExcelAndCtalk_All_Out = new byte[0];
    static byte[] commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out.length) {
				if(length < 1024 && commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out.length == 0) {
   					commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out = new byte[1024];
				} else {
   					commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out, 0, length);
			strReturn = new String(commonByteArray_BUCHEON_DurToExcelAndCtalk_All_Out, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_BUCHEON_DurToExcelAndCtalk_All_Out) {
 ...
 */
  ...
  try{
        if(key_tContextLoad_1!=null && "out_user".equals(key_tContextLoad_1))
        {
           context.out_user=value_tContextLoad_1;
        }

        if(key_tContextLoad_1!=null && "out_password".equals(key_tContextLoad_1))
        {
           context.out_password=value_tContextLoad_1;
        }

        if(key_tContextLoad_1!=null && "out_roomno".equals(key_tContextLoad_1))
        {
           context.out_roomno=value_tContextLoad_1;
        }
 ...
 */
	...
	currentComponent="tDBInput_1";
		...
		int tos_count_tDBInput_1 = 0;
		    ...
		    String dbquery_tDBInput_1 = "SELECT COUNT(*) CNT FROM (\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A"
+".INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n"
+"       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS R"
+"RGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORDDD "
+"AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM"
+".ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WH"
+"ERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드\n       '' PRCPNM,   -"
+"-처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 "
+"= E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PM"
+"CMPTBS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MI"
+"N(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   "
+"AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n--   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51'"
+" AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND EXISTS \n      (SELECT Y.TMP04\n        "
+" FROM PAM.SERVICECOOP Y\n        WHERE Y.TMP01 = 'ZIKA'\n          AND Y.TMP11 = 'Y'\n          AND Y.TMP02 = E.RRGSTNO1"
+"\n          AND Y.TMP03 = E.RRGSTNO2\n          AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n     "
+"  A.ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS"
+"_GETUSERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n      "
+" E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.M"
+"PPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORDDD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.O"
+"RDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLA"
+"G,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재"
+"진\n       '' PRCPCD,   --처방코드\n       '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM P"
+"AM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM ="
+"1 ) AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHSCND B\n WHERE A.INSTCD = NVL( '014', '014')\n   A"
+"ND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(S"
+"YSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = '"
+"O'\n--   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND "
+"A.INSTCD = E.INSTCD\n   AND B.INSTCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND "
+"A.ORDDD BETWEEN B.FROMDD AND B.TODD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         "
+"WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RR"
+"GSTNO2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       "
+"COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD)"
+" AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGST"
+"NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- "
+"휴대폰\n       A.ORGORDDD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n     "
+"  (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CD"
+"NM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드"
+"\n       '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP0"
+"1 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM"
+" PAM.PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHRARE B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SY"
+"SDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND"
+" HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n--   AND (A.INSUKIND IN ("
+"'11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND B"
+".INSTCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FROMDD AND "
+"B.TODD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n     "
+"      AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP1"
+"0 = E.PID)\nUNION ALL\nSELECT /*+ LEADING(EXOP OTPT OPRC) INDEX_SS(EXOP IX_MMODEXOP_09) */\n       DISTINCT '검사예약' AS GU"
+"BUN,\n       EXOP.RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD, OTPT.ORDDEPTCD, OTPT.ORDDD) AS ORDDEPTNM, --진료과\n  "
+"     COM.FN_ZS_GETUSERNM(OTPT.ORDDRID, OTPT.ORDDD) AS ORDDRNM,   --진료의사\n       OTPT.PID AS PID,    --등록번호\n       PTBS."
+"HNGNM,  --성명\n       PTBS.RRGSTNO1 || SUBSTR(PTBS.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PTBS.HOMETEL AS H"
+"OMETEL,    -- 연락처\n       PTBS.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       OTPT.ORGORDDD AS ACPTDD,    --접수일자\n       OTPT"
+".ORDDD AS ORDDD,    --진료일자\n       OTPT.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = "
+"'P0341' AND CDID = OTPT.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' A"
+"ND CDID = OTPT.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       OPRC.PRCPCD,   --처방코드\n       OPRC.PRCPNM,   --처방명칭\n       COM"
+".FN_ZS_GETDEPTNAME(OTPT.INSTCD, EXOP.EXECDEPTCD,OTPT.ORDDD) EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVIC"
+"ECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PTBS.RRGSTNO1 AND Y.TMP03 = PTBS.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1)"
+" AS REMFACT --전송내역\n  FROM EMR.MMODEXOP EXOP, EMR.MMOHOPRC OPRC, PAM.PMOHOTPT OTPT, PAM.PMCMPTBS PTBS\n WHERE OTPT.INSTC"
+"D = NVL( '014', '014')\n   AND OTPT.HISTSTAT IN ('R','T')\n--   AND OTPT.INSUKIND IN ('11','21','22')\n   AND OTPT.ORDDD"
+" BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623','YYYYMMDD'), -36),'YYYYMMDD') AND '20200623' --3년데이터\n   AND OTPT.ORDTYP"
+"E != 'S'\n   AND OTPT.INSTCD = PTBS.INSTCD\n   AND OTPT.PID = PTBS.PID\n   AND OTPT.INSTCD = EXOP.INSTCD\n   AND OTPT.PI"
+"D = EXOP.PID\n   AND OTPT.CRETNO = EXOP.ACTCRETNO\n   AND OTPT.ORDDD = EXOP.ACTORDDD\n   AND EXOP.INSTCD = OPRC.INSTCD\n"
+"   AND EXOP.PID = OPRC.PID\n   AND EXOP.PRCPDD = OPRC.PRCPDD\n   AND EXOP.PRCPNO = OPRC.PRCPNO\n   AND EXOP.EXECPRCPHIST"
+"CD = OPRC.PRCPHISTCD\n   AND OPRC.PRCPHISTCD = 'O'\n   AND EXOP.EXECPRCPHISTCD = 'O'\n   AND EXOP.RSRVFLAG = 'Y' --예약검사여"
+"부\n   AND EXOP.EXECDD = '00000000' --실시여부\n   AND EXOP.RSRVDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN"
+"(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   A"
+"ND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.T"
+"MP11 = 'Y'\n           AND Y.TMP02 = PTBS.RRGSTNO1\n           AND Y.TMP03 = PTBS.RRGSTNO2\n           AND Y.TMP10 = PTB"
+"S.PID)\nUNION ALL\nSELECT DISTINCT '검사예약' AS GUBUN,\n       A.EXECDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, "
+"A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID, A.ORDDD) AS ORDDRNM,   --진료의사\n       A"
+".PID AS PID,    --등록번호\n       B.HNGNM,  --성명\n       B.RRGSTNO1 || SUBSTR(B.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번"
+"호\n       B.HOMETEL AS HOMETEL,    -- 연락처\n       B.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,    --접수일자\n "
+"      A.ORDDD AS ORDDD,    --진료일자\n       '' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,"
+"  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT"
+" Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = B.RRGSTNO1 AND Y.TMP03 = B.RRGSTNO2 AND Y.TMP11 = '"
+"Y' AND ROWNUM =1) AS REMFACT --전송내역\n  FROM PAM.PAOHOSCL A, PAM.PMCMPTBS B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND"
+" A.EXECDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SY"
+"SDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.ORDTYPE != 'S'\n   AND A.CALCSTAT = 'Y'\n--   A"
+"ND A.INSUKIND = '11'\n   AND A.SUPPKIND != '01'\n   AND A.INSTCD = B.INSTCD\n   AND A.PID = B.PID\n   AND A.RCPTDD BETWE"
+"EN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND A.ORDDD BETWEEN TO_"
+"CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND EXISTS\n       (SELECT Y."
+"TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y"
+".TMP02 = B.RRGSTNO1\n           AND Y.TMP03 = B.RRGSTNO2\n           AND Y.TMP10 = B.PID)\n GROUP BY A.PID, A.ORDDD, A.C"
+"RETNO, A.INSTCD, A.ORDTYPE, EXECDD, RRGSTNO1||RRGSTNO2, HNGNM, A.ORDDEPTCD, A.SUPPKIND, A.INSUKIND, A.ORDDRID, B.RRGSTNO"
+"1, B.RRGSTNO2, B.HOMETEL, B.MPPHONTEL\nUNION ALL\nSELECT 'DSC예약' AS GUBUN,\n       A.INDD AS RSRVDD,\n       COM.FN_ZS_G"
+"ETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.INDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.MEDISPCLID, A.INDD) AS ORDDR"
+"NM,   --진료의사\n       A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'*****"
+"*' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.IN"
+"DD AS ACPTDD,    --접수일자\n       A.INDD AS ORDDD,    --진료일자\n       A.INTM AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예"
+"약구분\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEP"
+"TNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03"
+" = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n   FROM PAM.PMIHINPT A, PAM.PMCMPTBS E\n WHERE A.INST"
+"CD = NVL( '014', '014')\n   AND A.INDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMH"
+"SCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT = 'Y'\n  "
+" AND A.ORDTYPE = 'D'\n   AND A.DSCHDD = '99991231'\n   AND A.CALCDD = '00000000' --입실체크 안된내역만.\n   AND A.CALCTM = '0000'"
+"\n--   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n"
+"   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AN"
+"D Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PI"
+"D)\nUNION ALL\nSELECT DISTINCT '입원예약' AS GUGUN,\n       A.PATINHOPEDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD,"
+" A.ORDDEPTCD, A.PATINHOPEDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID, A.INDIRECDD) AS ORDDRNM,   --진료의"
+"사\n       A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGST"
+"NO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,  "
+"  --접수일자\n       A.PATINHOPEDD AS ORDDD,    --진료일자\n       '' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       "
+"'' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부"
+"서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO"
+"2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMIHADRV A, PAM.PMCMPTBS B, PAM.PMIHINPT C, PAM.PMCMPT"
+"BS E\n WHERE A.PID = B.PID\n   AND A.INSTCD = B.INSTCD\n   AND A.INSTCD = '014'\n   AND A.INSTCD = E.INSTCD\n   AND A.PI"
+"D = E.PID\n   AND C.INSTCD(+) = A.INSTCD\n   AND C.PID(+) = A.PID\n   AND C.HISTSTAT(+) = 'Y'\n   AND C.INDSCHACPTSTAT(+"
+") = 'A'\n   AND C.DSCHDD(+) = '99991231'\n   AND C.ORDTYPE(+) = 'I'\n   AND A.INRSRVSTAT IN ('S','R')\n   AND A.INFLAG ="
+" 'G'\n   AND A.ERREGLYN IN ('I','F','R','G','D')\n   AND A.PATINHOPEDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (S"
+"ELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '"
+"0')\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n         "
+"  AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = "
+"E.PID)\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEP"
+"TNAME(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GE"
+"TUSERNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID,    --등록번호\n    "
+"   PATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       "
+"PATM.GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYY"
+"YMMDD') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT"
+"_TM AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG)"
+" AS RSRVFLAG,  --예약구분\n       (CASE WHEN ACPT.RMDEM_YN = 'Y' THEN '재진' ELSE '초진' END) AS FSEXAMFLAG,  --초재진\n       '' A"
+"S PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SE"
+"RVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = "
+"1) AS REMFACT --전송내역\n  FROM HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC.HCADOTPT OTPT\n WHERE ACPT.INST_CD = NVL( '014', '01"
+"4')\n   AND ACPT.ACPT_DD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASE"
+"DD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND ACPT.INST_CD = PATM.INST_CD\n   AN"
+"D ACPT.PID = PATM.PID\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 "
+"= 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n  "
+"         AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT IN ('C')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기"
+", I 판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   AND ACPT.HC_ACPT_SEQ_NO = OTPT.HC_ACPT_SEQ_NO\nUNION ALL\nSELECT '건진예약'"
+" AS GUBUN,\n       TO_CHAR(RCNT.RSLT_CNST_DD, 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PR"
+"CP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_"
+"CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID,    --등록번호\n       PATM.CLNT_NM AS HNGNM,  "
+"--성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL"
+",    -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일"
+"자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT_TM AS ORDTM,    --시간\n    "
+"   (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n    "
+"   '상담' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   "
+"--시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PA"
+"TM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HCAMRCNT RCNT, HC.HCAMACPT ACPT, HC.HCAMPATM P"
+"ATM, HC.HCADOTPT OTPT\n WHERE RCNT.INST_CD = NVL( '014', '014')\n   AND RCNT.RSLT_CNST_DD BETWEEN TO_CHAR(SYSDATE + 0.5,"
+" 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = "
+"'Y' AND DUTFLAG = '0')\n   AND RCNT.RSLT_CNST_VALI_CNST_CD = 'O'\n   AND EXISTS \n       (SELECT 1\n          FROM HC.HC"
+"BMCODE SUB\n         WHERE SUB.INST_CD = RCNT.INST_CD\n           AND SUB.GRUP_CD_ID = 'A0003'\n           AND SUB.CD_VA"
+"L_NM LIKE '%내원%'\n           AND SUB.USE_YN = 'Y'\n           AND RCNT.RSLT_CNST_DD BETWEEN SUB.USE_FRDD AND SUB.USE_TOD"
+"D\n           AND SUB.CD_VAL = RCNT.RSLT_CNST_FLAG)\n   AND RCNT.INST_CD = ACPT.INST_CD\n   AND RCNT.HC_ACPT_SEQ_NO = AC"
+"PT.HC_ACPT_SEQ_NO\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n   AND EXISTS\n       (SELECT Y.TMP04"
+"\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP0"
+"2 = PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT NOT IN "
+"('B','D','F')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   "
+"AND ACPT.HC_ACPT_SEQ_NO = OTPT.HC_ACPT_SEQ_NO\n)";
...
userStruct user = new userStruct();
	...
	currentComponent="tFileOutputExcel_2";
		...
		String fileName_tFileOutputExcel_2="D:/data/DurBatch/해외감염병정보_외래간호_"+TalendDate.formatDate("yyyyMMddHH", new Date()) + ".xls";
...
+"\n  FROM DUAL\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INS"
+"TCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n    "
+"   A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGST"
+"NO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORDDD AS A"
+"CPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBC"
+"MCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE "
+"CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드\n       '' PRCPNM,   --처방명"
+"칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E."
+"RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMCMPT"
+"BS E\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BA"
+"SEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND "
+"A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n--   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND"
+" RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND EXISTS \n      (SELECT Y.TMP04\n         FRO"
+"M PAM.SERVICECOOP Y\n        WHERE Y.TMP01 = 'ZIKA'\n          AND Y.TMP11 = 'Y'\n          AND Y.TMP02 = E.RRGSTNO1\n  "
+"        AND Y.TMP03 = E.RRGSTNO2\n          AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A."
+"ORDDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GET"
+"USERNM(A.ORDDRID,A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.R"
+"RGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHO"
+"NTEL AS MPPHONTEL,    -- 휴대폰\n       A.ORGORDDD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM"
+" AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  "
+"--예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n "
+"      '' PRCPCD,   --처방코드\n       '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.S"
+"ERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) "
+"AS REMFACT --전송내역\n  FROM PAM.PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHSCND B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A"
+".ORDDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDA"
+"TE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n"
+"--   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.IN"
+"STCD = E.INSTCD\n   AND B.INSTCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.OR"
+"DDD BETWEEN B.FROMDD AND B.TODD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHER"
+"E Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTN"
+"O2\n           AND Y.TMP10 = E.PID)\nUNION ALL\nSELECT DISTINCT '진료예약' AS GUBUN,\n       A.ORDDD AS RSRVDD,\n       COM."
+"FN_ZS_GETDEPTNAME(A.INSTCD, A.ORDDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID,A.ORDDD) AS "
+"ORDDRNM,   --진료의사\n       A.PID AS PID,    --등록번호\n       HNGNM AS HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,"
+"0,1)||'******' AS RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰"
+"\n       A.ORGORDDD AS ACPTDD,    --접수일자\n       A.ORDDD AS ORDDD,    --진료일자\n       A.ORDTM AS ORDTM,    --시간\n       (S"
+"ELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0341' AND CDID = A.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM F"
+"ROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND CDID = A.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       '' PRCPCD,   --처방코드\n  "
+"     '' PRCPNM,   --처방명칭\n       '' EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = "
+"'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM"
+".PMOHOTPT A, PAM.PMCMPTBS E, PAM.PMCHRARE B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.ORDDD BETWEEN TO_CHAR(SYSDAT"
+"E + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HIS"
+"TSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT IN ( 'R', 'T' )\n   AND A.ORDTYPE = 'O'\n--   AND (A.INSUKIND IN ('11'"
+", '21', '22') OR (A.INSUKIND = '51' AND RSRVFLAG = '5'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   AND B.INS"
+"TCD = E.INSTCD\n   AND B.RRGSTNO1 = E.RRGSTNO1\n   AND B.RRGSTNO2 = E.RRGSTNO2\n   AND A.ORDDD BETWEEN B.FROMDD AND B.TO"
+"DD\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n         "
+"  AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = "
+"E.PID)\nUNION ALL\nSELECT /*+ LEADING(EXOP OTPT OPRC) INDEX_SS(EXOP IX_MMODEXOP_09) */\n       DISTINCT '검사예약' AS GUBUN,"
+"\n       EXOP.RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(OTPT.INSTCD, OTPT.ORDDEPTCD, OTPT.ORDDD) AS ORDDEPTNM, --진료과\n      "
+" COM.FN_ZS_GETUSERNM(OTPT.ORDDRID, OTPT.ORDDD) AS ORDDRNM,   --진료의사\n       OTPT.PID AS PID,    --등록번호\n       PTBS.HNGN"
+"M,  --성명\n       PTBS.RRGSTNO1 || SUBSTR(PTBS.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PTBS.HOMETEL AS HOMET"
+"EL,    -- 연락처\n       PTBS.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       OTPT.ORGORDDD AS ACPTDD,    --접수일자\n       OTPT.ORD"
+"DD AS ORDDD,    --진료일자\n       OTPT.ORDTM AS ORDTM,    --시간\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P03"
+"41' AND CDID = OTPT.RSRVFLAG) AS RSRVFLAG,  --예약구분\n       (SELECT CDNM FROM COM.ZBCMCODE WHERE CDGRUPID = 'P0021' AND C"
+"DID = OTPT.FSEXAMFLAG) AS FSEXAMFLAG,  --초재진\n       OPRC.PRCPCD,   --처방코드\n       OPRC.PRCPNM,   --처방명칭\n       COM.FN_"
+"ZS_GETDEPTNAME(OTPT.INSTCD, EXOP.EXECDEPTCD,OTPT.ORDDD) EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOO"
+"P Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PTBS.RRGSTNO1 AND Y.TMP03 = PTBS.RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS "
+"REMFACT --전송내역\n  FROM EMR.MMODEXOP EXOP, EMR.MMOHOPRC OPRC, PAM.PMOHOTPT OTPT, PAM.PMCMPTBS PTBS\n WHERE OTPT.INSTCD = "
+"NVL( '014', '014')\n   AND OTPT.HISTSTAT IN ('R','T')\n--   AND OTPT.INSUKIND IN ('11','21','22')\n   AND OTPT.ORDDD BET"
+"WEEN TO_CHAR(ADD_MONTHS(TO_DATE( '20200623','YYYYMMDD'), -36),'YYYYMMDD') AND '20200623' --3년데이터\n   AND OTPT.ORDTYPE !="
+" 'S'\n   AND OTPT.INSTCD = PTBS.INSTCD\n   AND OTPT.PID = PTBS.PID\n   AND OTPT.INSTCD = EXOP.INSTCD\n   AND OTPT.PID = "
+"EXOP.PID\n   AND OTPT.CRETNO = EXOP.ACTCRETNO\n   AND OTPT.ORDDD = EXOP.ACTORDDD\n   AND EXOP.INSTCD = OPRC.INSTCD\n   A"
+"ND EXOP.PID = OPRC.PID\n   AND EXOP.PRCPDD = OPRC.PRCPDD\n   AND EXOP.PRCPNO = OPRC.PRCPNO\n   AND EXOP.EXECPRCPHISTCD ="
+" OPRC.PRCPHISTCD\n   AND OPRC.PRCPHISTCD = 'O'\n   AND EXOP.EXECPRCPHISTCD = 'O'\n   AND EXOP.RSRVFLAG = 'Y' --예약검사여부\n "
+"  AND EXOP.EXECDD = '00000000' --실시여부\n   AND EXOP.RSRVDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BAS"
+"EDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND E"
+"XISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11"
+" = 'Y'\n           AND Y.TMP02 = PTBS.RRGSTNO1\n           AND Y.TMP03 = PTBS.RRGSTNO2\n           AND Y.TMP10 = PTBS.PI"
+"D)\nUNION ALL\nSELECT DISTINCT '검사예약' AS GUBUN,\n       A.EXECDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.OR"
+"DDEPTCD, A.ORDDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID, A.ORDDD) AS ORDDRNM,   --진료의사\n       A.PID"
+" AS PID,    --등록번호\n       B.HNGNM,  --성명\n       B.RRGSTNO1 || SUBSTR(B.RRGSTNO2,0,1)||'******' AS RRGSTNO,   --주민번호\n "
+"      B.HOMETEL AS HOMETEL,    -- 연락처\n       B.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,    --접수일자\n     "
+"  A.ORDDD AS ORDDD,    --진료일자\n       '' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' AS FSEXAMFLAG,  --"
+"초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.T"
+"MP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = B.RRGSTNO1 AND Y.TMP03 = B.RRGSTNO2 AND Y.TMP11 = 'Y' A"
+"ND ROWNUM =1) AS REMFACT --전송내역\n  FROM PAM.PAOHOSCL A, PAM.PMCMPTBS B\n WHERE A.INSTCD = NVL( '014', '014')\n   AND A.E"
+"XECDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDAT"
+"E - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.ORDTYPE != 'S'\n   AND A.CALCSTAT = 'Y'\n--   AND A"
+".INSUKIND = '11'\n   AND A.SUPPKIND != '01'\n   AND A.INSTCD = B.INSTCD\n   AND A.PID = B.PID\n   AND A.RCPTDD BETWEEN T"
+"O_CHAR(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND A.ORDDD BETWEEN TO_CHAR"
+"(ADD_MONTHS(TO_DATE( '20200623', 'YYYYMMDD')-1, -12), 'YYYYMMDD') AND '20200623'-1\n   AND EXISTS\n       (SELECT Y.TMP0"
+"4\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP"
+"02 = B.RRGSTNO1\n           AND Y.TMP03 = B.RRGSTNO2\n           AND Y.TMP10 = B.PID)\n GROUP BY A.PID, A.ORDDD, A.CRETN"
+"O, A.INSTCD, A.ORDTYPE, EXECDD, RRGSTNO1||RRGSTNO2, HNGNM, A.ORDDEPTCD, A.SUPPKIND, A.INSUKIND, A.ORDDRID, B.RRGSTNO1, B"
+".RRGSTNO2, B.HOMETEL, B.MPPHONTEL\nUNION ALL\nSELECT 'DSC예약' AS GUBUN,\n       A.INDD AS RSRVDD,\n       COM.FN_ZS_GETDE"
+"PTNAME(A.INSTCD, A.ORDDEPTCD, A.INDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.MEDISPCLID, A.INDD) AS ORDDRNM, "
+"  --진료의사\n       A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' A"
+"S RRGSTNO,   --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       A.INDD A"
+"S ACPTDD,    --접수일자\n       A.INDD AS ORDDD,    --진료일자\n       A.INTM AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분"
+"\n       '' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,"
+"   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E"
+".RRGSTNO2 AND Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n   FROM PAM.PMIHINPT A, PAM.PMCMPTBS E\n WHERE A.INSTCD ="
+" NVL( '014', '014')\n   AND A.INDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH "
+"WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND A.HISTSTAT = 'Y'\n   AND"
+" A.ORDTYPE = 'D'\n   AND A.DSCHDD = '99991231'\n   AND A.CALCDD = '00000000' --입실체크 안된내역만.\n   AND A.CALCTM = '0000'\n--"
+"   AND (A.INSUKIND IN ('11', '21', '22') OR (A.INSUKIND = '51'))\n   AND A.PID = E.PID\n   AND A.INSTCD = E.INSTCD\n   A"
+"ND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y."
+"TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PID)\n"
+"UNION ALL\nSELECT DISTINCT '입원예약' AS GUGUN,\n       A.PATINHOPEDD AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(A.INSTCD, A.O"
+"RDDEPTCD, A.PATINHOPEDD) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(A.ORDDRID, A.INDIRECDD) AS ORDDRNM,   --진료의사\n "
+"      A.PID AS PID,    --등록번호\n       E.HNGNM,  --성명\n       E.RRGSTNO1 || SUBSTR(E.RRGSTNO2,0,1)||'******' AS RRGSTNO, "
+"  --주민번호\n       E.HOMETEL AS HOMETEL,    -- 연락처\n       E.MPPHONTEL AS MPPHONTEL,    -- 휴대폰\n       '' AS ACPTDD,    --"
+"접수일자\n       A.PATINHOPEDD AS ORDDD,    --진료일자\n       '' AS ORDTM,    --시간\n       '' AS RSRVFLAG,  --예약구분\n       '' A"
+"S FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n "
+"      (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = E.RRGSTNO1 AND Y.TMP03 = E.RRGSTNO2 AN"
+"D Y.TMP11 = 'Y' AND ROWNUM =1 ) AS REMFACT --전송내역\n  FROM PAM.PMIHADRV A, PAM.PMCMPTBS B, PAM.PMIHINPT C, PAM.PMCMPTBS E"
+"\n WHERE A.PID = B.PID\n   AND A.INSTCD = B.INSTCD\n   AND A.INSTCD = '014'\n   AND A.INSTCD = E.INSTCD\n   AND A.PID = "
+"E.PID\n   AND C.INSTCD(+) = A.INSTCD\n   AND C.PID(+) = A.PID\n   AND C.HISTSTAT(+) = 'Y'\n   AND C.INDSCHACPTSTAT(+) = "
+"'A'\n   AND C.DSCHDD(+) = '99991231'\n   AND C.ORDTYPE(+) = 'I'\n   AND A.INRSRVSTAT IN ('S','R')\n   AND A.INFLAG = 'G'"
+"\n   AND A.ERREGLYN IN ('I','F','R','G','D')\n   AND A.PATINHOPEDD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELEC"
+"T MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')"
+"\n   AND EXISTS\n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AN"
+"D Y.TMP11 = 'Y'\n           AND Y.TMP02 = E.RRGSTNO1\n           AND Y.TMP03 = E.RRGSTNO2\n           AND Y.TMP10 = E.PI"
+"D)\nUNION ALL\nSELECT '건진예약' AS GUBUN,\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAM"
+"E(ACPT.INST_CD, ACPT.PRCP_DEPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSE"
+"RNM(ACPT.PRCP_DR_ID,TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID,    --등록번호\n       P"
+"ATM.CLNT_NM AS HNGNM,  --성명\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PATM"
+".GNRL_TEL_NO AS HOMETEL,    -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMD"
+"D') AS ACPTDD,    --접수일자\n       TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT_TM "
+"AS ORDTM,    --시간\n       (SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG) AS "
+"RSRVFLAG,  --예약구분\n       (CASE WHEN ACPT.RMDEM_YN = 'Y' THEN '재진' ELSE '초진' END) AS FSEXAMFLAG,  --초재진\n       '' AS PR"
+"CPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행부서\n       (SELECT Y.TMP31 FROM PAM.SERVIC"
+"ECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PATM.RES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) A"
+"S REMFACT --전송내역\n  FROM HC.HCAMACPT ACPT, HC.HCAMPATM PATM, HC.HCADOTPT OTPT\n WHERE ACPT.INST_CD = NVL( '014', '014')"
+"\n   AND ACPT.ACPT_DD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YYYYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD >"
+" TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' AND DUTFLAG = '0')\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND AC"
+"PT.PID = PATM.PID\n   AND EXISTS \n       (SELECT Y.TMP04\n          FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'Z"
+"IKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n      "
+"     AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT IN ('C')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I "
+"판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   AND ACPT.HC_ACPT_SEQ_NO = OTPT.HC_ACPT_SEQ_NO\nUNION ALL\nSELECT '건진예약' AS "
+"GUBUN,\n       TO_CHAR(RCNT.RSLT_CNST_DD, 'YYYYMMDD') AS RSRVDD,\n       COM.FN_ZS_GETDEPTNAME(ACPT.INST_CD, ACPT.PRCP_D"
+"EPT_CD, TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD')) AS ORDDEPTNM, --진료과\n       COM.FN_ZS_GETUSERNM(ACPT.PRCP_DR_ID,TO_CHAR"
+"(ACPT.ACPT_DD, 'YYYYMMDD')) AS ORDDRNM,   --진료의사\n       ACPT.PID AS PID,    --등록번호\n       PATM.CLNT_NM AS HNGNM,  --성명"
+"\n       PATM.RES_NO1 || SUBSTR(PATM.RES_NO2,0,1)||'******' AS RRGSTNO,   --주민번호\n       PATM.GNRL_TEL_NO AS HOMETEL,   "
+" -- 연락처\n       PATM.MPPHON_NO AS MPPHONTEL,    -- 휴대폰\n       TO_CHAR(ACPT.ACPT_DD, 'YYYYMMDD') AS ACPTDD,    --접수일자\n "
+"      TO_CHAR(OTPT.OPD_MEDTRT_DD, 'YYYYMMDD') AS ORDDD,    --진료일자\n       OTPT.OPD_MEDTRT_TM AS ORDTM,    --시간\n       ("
+"SELECT CD_VAL_NM FROM HC.HCBMCODE WHERE GRUP_CD_ID = 'A0119' AND CD_VAL = ACPT.RSRV_FLAG) AS RSRVFLAG,  --예약구분\n       '"
+"상담' AS FSEXAMFLAG,  --초재진\n       '' AS PRCPCD,   --처방코드\n       '' AS PRCPNM,   --처방명칭\n       '' AS EXECDEPTNM,   --시행"
+"부서\n       (SELECT Y.TMP31 FROM PAM.SERVICECOOP Y WHERE Y.TMP01 = 'ZIKA' AND Y.TMP02 = PATM.RES_NO1 AND Y.TMP03 = PATM.R"
+"ES_NO2 AND Y.TMP11 = 'Y' AND ROWNUM = 1) AS REMFACT --전송내역\n  FROM HC.HCAMRCNT RCNT, HC.HCAMACPT ACPT, HC.HCAMPATM PATM,"
+" HC.HCADOTPT OTPT\n WHERE RCNT.INST_CD = NVL( '014', '014')\n   AND RCNT.RSLT_CNST_DD BETWEEN TO_CHAR(SYSDATE + 0.5, 'YY"
+"YYMMDD') AND (SELECT MIN(BASEDD) FROM PAM.PMBMHSCH WHERE BASEDD > TO_CHAR(SYSDATE - 0.5, 'YYYYMMDD') AND HISTSTAT = 'Y' "
+"AND DUTFLAG = '0')\n   AND RCNT.RSLT_CNST_VALI_CNST_CD = 'O'\n   AND EXISTS \n       (SELECT 1\n          FROM HC.HCBMCO"
+"DE SUB\n         WHERE SUB.INST_CD = RCNT.INST_CD\n           AND SUB.GRUP_CD_ID = 'A0003'\n           AND SUB.CD_VAL_NM"
+" LIKE '%내원%'\n           AND SUB.USE_YN = 'Y'\n           AND RCNT.RSLT_CNST_DD BETWEEN SUB.USE_FRDD AND SUB.USE_TODD\n "
+"          AND SUB.CD_VAL = RCNT.RSLT_CNST_FLAG)\n   AND RCNT.INST_CD = ACPT.INST_CD\n   AND RCNT.HC_ACPT_SEQ_NO = ACPT.H"
+"C_ACPT_SEQ_NO\n   AND ACPT.INST_CD = PATM.INST_CD\n   AND ACPT.PID = PATM.PID\n   AND EXISTS\n       (SELECT Y.TMP04\n  "
+"        FROM PAM.SERVICECOOP Y\n         WHERE Y.TMP01 = 'ZIKA'\n           AND Y.TMP11 = 'Y'\n           AND Y.TMP02 = "
+"PATM.RES_NO1\n           AND Y.TMP03 = PATM.RES_NO2\n           AND Y.TMP10 = PATM.PID)\n   AND ACPT.HC_STAT NOT IN ('B'"
+",'D','F')--A 가예약, B 가예약취소, C 예약, D 예약취소, E 접수, F 접수취소, G 결과, H 판정대기, I 판정완료\n   AND ACPT.INST_CD = OTPT.INST_CD\n   AND "
+"ACPT.HC_ACPT_SEQ_NO = OTPT.HC_ACPT_SEQ_NO";
...
final Process ps_tSystem_1 = runtime_tSystem_1.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar " + context.out_user + " " + context.out_password + " " + context.out_roomno + " D:/data/DurBatch/해외감염병정보_외래간호_"+TalendDate.formatDate("yyyyMMddHH", new Date()) + ".xls" ,env_tSystem_1,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
...
final Process ps_tSystem_4 = runtime_tSystem_4.exec( "cmd /c java -Dfile.encoding=UTF-8 -jar CompactCTalkSender.jar " + context.out_user + " " + context.out_password + " " + context.out_roomno + " D:/data/DurBatch/CompactCTalkSender/none.txt" ,env_tSystem_4,new java.io.File("D:/data/DurBatch/CompactCTalkSender"));
 ...
 */
    ...
    public static void main(String[] args){
        final DurToExcelAndCtalk_All_Out DurToExcelAndCtalk_All_OutClass = new DurToExcelAndCtalk_All_Out();

        int exitCode = DurToExcelAndCtalk_All_OutClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = DurToExcelAndCtalk_All_Out.class.getClassLoader().getResourceAsStream("bucheon/durtoexcelandctalk_all_out_0_5/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = DurToExcelAndCtalk_All_Out.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
            ...
            class ContextProcessing {
                private void processContext_0() {
                        context.setContextType("out_user", "id_String");
                            context.out_user=(String) context.getProperty("out_user");
                        context.setContextType("out_password", "id_String");
                            context.out_password=(String) context.getProperty("out_password");
                        context.setContextType("out_roomno", "id_String");
                            context.out_roomno=(String) context.getProperty("out_roomno");
                } 
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("out_user")) {
                context.out_user = (String) parentContextMap.get("out_user");
            }if (parentContextMap.containsKey("out_password")) {
                context.out_password = (String) parentContextMap.get("out_password");
            }if (parentContextMap.containsKey("out_roomno")) {
                context.out_roomno = (String) parentContextMap.get("out_roomno");
            }
        }
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : DurToExcelAndCtalk_All_Out");
        }
...
/************************************************************************************************
 *     157952 characters generated by Talend Open Studio for Data Integration 
 *     on the 2022년 4월 28일 (목) 오전 10시 56분 43초
 ************************************************************************************************/